...
11. 作为木下，笔记发布前为「草稿」状态，仅自己在管理后台可见；手动点击「发布」后公开可见并出现在首页时间线和 RSS 中。
...
- **发布流程接缝**：草稿→发布状态变更正确，发布后首页/RSS 可发现。
...
- 与 /blog 的关系：两者独立但共享全局视觉基调和 RSS。聚合首页混排两者内容。
...
> **Triage**: 
eady-for-agent — PRD 已完整，Phase 3 架构设计时裁决 SSG vs SSR 冲突和认证方案。
