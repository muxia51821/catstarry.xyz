...
assert.match(html, /<details class="panel panel--span-2 access-disclosure" data-pane="records" data-access-panel hidden>/, 'security access log must remain an admin-only collapsed disclosure');
...
assert.doesNotMatch(operationsCss, /\.operation-history-ready \[data-access-panel\]/, 'Operation History must not hide the auxiliary security access log');
assert.match(operationsCss, /\.operation-workbook-review-ready \[data-import-review-panel\]/);
assert.doesNotMatch(operationsCss, /^\[data-access-panel\][\s,]/m, 'security access log must not be hidden by Operation History CSS');
...
assert.match(operationsRoute, /circuit-resolved:/);
assert.match(operationsRoute, /system:circuit-state/);
