...
assert.equal(all.rows[0].event_key, 'reconciliation:103', 'latest real reconciliation should lead the activity stream');
...
const dividendRows = rows({ filter: { ...emptyFilter, kind: 'account_event' } }).rows;
assert.equal(humanizeActivity(dividendRows[0]).title, '现金分红 · 宁德时代');
assert.match(humanizeActivity(dividendRows[0]).summary, /\+¥141\.1/);
