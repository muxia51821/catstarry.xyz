...
3. hover 或 Tab 聚焦 Blog / Feed / Learn / Projects：标签增强，且对应 `active` / `stable` 伴星只回应一次。其低频运动应是长时间静止后的短弧移动；`dormant` 完全静止。活动状态以仅屏幕阅读器可见的描述关联到对应星球，不在星图中增加状态文字。
...
10. 在 About 邻近区域操作 companion：总览先读成低音量 Klein Blue 小天体；进入引力范围后再显出豹猫。桌面使用两次独立点击：第一次蓄能，第二次局部爆开后进入相同 About 展开态；这不是浏览器 `dblclick`。触控端单击豹猫直接爆开后进入 About；关闭后沿用既有回收逻辑。
...
- HAS 伴星周期：active `24s`、stable `32s`；只有末尾 `18% / 12%` 时间执行短弧移动；dormant 静止。仅在 fine pointer 且未启用 reduced-motion 时运行；触控端保留静态三态材质。
...
- HAS 与豹猫本轮只完成状态可靠性、触控、reduced-motion 和无障碍路径；轮廓、材质、尺寸比例、轨道、rim、尘埃、身体比例及静态关键状态仍是视觉待校准，不是视觉定稿。
