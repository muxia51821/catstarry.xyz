...
2. 准备真实项目视觉证据。UI 项目优先使用真实截图；非 UI 项目可以使用真实、可说明项目本体的其他证据图。建议 `1600×1000`、WebP、单文件不超过 350 KB，放入 `public/assets/projects/<projectId>.webp`。不得用行星图、空白图或生成式装饰 filler 代替真实项目证据。当前 `npm run project:add` helper 仍要求 `screenshot` 字段指向 `/assets/projects/` 下的真实图片；这是当前 helper contract，不是“所有 Project 永远必须有 UI screenshot”的 Product 规则。若未来项目确实没有合适视觉证据，应单独调整 implementation contract，而不是添加虚假占位图。
...
- 普通索引编辑、截图／项目证据图替换、构建、部署失败与重复部署不创建 Feed Footprint。
