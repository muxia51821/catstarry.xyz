...
- **Blog**：第一次 lifecycle manifest 初始化只建立 baseline、不回填历史；之后 deploy sync 保留已有 owner state，并可为 never-published、首次处于 `published` 的新 source 创建幂等 `first-production-v1` footprint。Owner lifecycle PATCH 也可能完成同一首次 publication。每次 production sync 携带 exact release identity；旧 generation 或同 generation 不同 SHA 会在 lifecycle mutation 前被拒绝。Site deploy preflight 另外保证当前 runtime-published slug 仍存在于 candidate source；source frontmatter state 不替代 runtime lifecycle authority。
