...
> 本清单只描述用户可观察的 Home 行为。星球材质、豹猫星座和具体动效细节由 DESIGN.md 与 Phase 4 原型验证决定。
...
- [ ] 豹猫星座作为可选彩蛋：第一次点击蓄能，第二次点击后仅豹猫粒子局部爆开，并进入同一 About 展开态；About 星球本身不爆炸。
...
- [ ] 活跃与稳定卫星可沿低频椭圆轨道缓慢公转，并在经过主星前后时呈现遮挡关系；稳定卫星的能量、尺度和运动强度弱于活跃卫星；沉寂卫星保持更低能量的静态或近静态状态但仍可辨认。
- [ ] 活跃与稳定卫星仅在进入视野、悬停或键盘聚焦时作一次克制短脉冲；沉寂卫星不发出辉光脉冲。所有状态不得自动聚焦、持续闪烁或抢占主星视角。
...
- [ ] Phase 4.3 已选五颗星球的 Overview / Focus / Mobile 三槽资产能分别读出已锁定主地貌，并保持同一星球身份、主光方向、主地貌和校色，不出现明显换图感。
- [ ] selected assets 可通过集中路径替换，不需要修改 Star Map、Focus 或 action 的交互逻辑。
- [ ] 当前 selected assets 代表 Phase 4.3 设计身份闭合，不代表 Phase 5 的最终 2x / CDN / R2 交付规格已经完成。
- [ ] Phase 5 正式接入时必须验证真实加载策略、LCP / CLS、资源优先级和大屏 2x 需求；这不重新打开五颗星球的身份设计。
