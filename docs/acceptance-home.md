...
- [ ] 星球可在各自语义区域内做少量人工编排变化，但不会在每次打开页面时随机换位；豹猫 companion 始终跟随 About。
...
- [ ] 活跃卫星可低频沿局部开放弧微移；稳定卫星的移动更弱更慢；沉寂卫星保持静止但仍可辨认。
- [ ] 活跃与稳定卫星仅在进入视野、悬停或键盘聚焦时作一次克制反馈；沉寂卫星不发出辉光脉冲。所有状态不得完整公转、自动巡航、自动聚焦或持续闪烁。
- [ ] 活动状态投影缺失或无效时，四颗功能星球都不显示活动卫星；不得将不可用状态伪装成沉寂。

## 7. 可访问性与设备适配

- [ ] 使用触控设备时，悬停所承载的关键信息可通过点按获得。
- [ ] 启用“减少动态效果”后，滚动与进入动效降级，不阻碍导航或阅读。
- [ ] 启用“减少动态效果”后，活动卫星停止微移与辉光脉冲，但三态仍能通过静态材质、轨道残留和文字状态区分。
- [ ] 精细指针设备上，鼠标流星尾在 Home 清晰但克制；在 Content 页面更短更淡；Finance 与“减少动态效果”模式下关闭。
- [ ] 手机、平板与桌面端均能完成星图浏览、进入板块和 About 展开。

## 8. 星球候选资产

- [ ] Phase 4.2 的五颗 Overview 候选能分别读出已锁定主地貌；不合格图片可作为校准占位，但不能标记为视觉通过。
- [ ] Focus 与 Mobile 候选保持与 Overview 相同的星球身份、主光方向、主地貌和校色，不出现明显换图感。
- [ ] 候选资产可通过集中路径替换，不需要修改 Star Map、Focus 或 action 的交互逻辑。
- [ ] Phase 4.2 不要求最终生产分辨率和批量导出，但进入正式生产前必须替换所有未通过目测的占位资产。
