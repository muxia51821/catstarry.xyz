...
| 4 UI/原型  | 独立（Phase 3 确认后） | 🟡 HAS 返回 4.1 已完成；Phase 4.2 可恢复 |
...
当前真实入口：Home Activity Signal 定向 Phase 2/3 与 HAS 返回 Phase 4.1 视觉重锁均已完成。现在应 fork Phase 4.2 隔离原型任务：只用模拟 `active` / `stable` / `dormant` 校准信号卫星、星图、Planet Push、About 原地展开、豹猫卫星、鼠标流星尾与相关 calibration 参数；不接入真实投影、不写生产代码、不修改 canonical CSS、不进入 Phase 4.3。
