...
| 4 UI/原型  | 独立（Phase 3 确认后） | 🟡 4.1 已完成；Astro 7 基线已确认；4.2 可启动 |
...
当前真实入口：Phase 4.1 已完成，Astro 7 定向依赖基线迁移也已完成并由流程治理确认。现在应 fork Phase 4.2 隔离原型任务：只验证星图、Planet Push、About 原地展开、豹猫卫星、鼠标流星尾与相关 calibration 参数；不写生产代码，不修改 canonical CSS，不进入 Phase 4.3。
