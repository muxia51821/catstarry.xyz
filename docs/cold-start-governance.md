...
DESIGN.md                           — Design 2.1 已锁定；Phase 4.2 只能校准已锁定交互与视觉参数
```
...
| 4 UI/原型  | 独立（Phase 3 确认后） | 🟡 Design 2.1 极小重锁已完成；Phase 4.2 进行中 |
...
当前真实入口：Home Activity Signal 定向 Phase 2/3、HAS 返回 Phase 4.1 视觉重锁与 2026-07-18 Design 2.1 极小交互重锁均已完成。继续 Phase 4.2 隔离原型任务：验证 Drift 语义构图、`Star Map → Focus → action`、默认滚动 Focus 序列、五颗星球候选资产连续性，并只用模拟 `active` / `stable` / `dormant` 校准信号卫星、About 原地展开、豹猫 companion、鼠标流星尾与相关 calibration 参数；不接入真实投影、不写生产代码、不修改 canonical CSS、不进入 Phase 4.3。
