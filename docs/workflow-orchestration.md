...
- 会话长度不设限；但**生命周期切换点必须显式声明**——从实现转入发布、数据操作或新需求时，先输出上一阶段 closure ledger 并确认新阶段的授权边界，再继续。
- context compaction 发生后 → 先输出五状态 ledger（Merged / Deployed / DataChanged / Verified / Next authorized）再恢复工作。
- 新需求先落 `.scratch` ticket 再动工，不在验收尾部顺带实现。
