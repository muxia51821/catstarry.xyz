...
- scope 清楚、可在当前会话完成的小型修复不要求创建 ticket；复杂任务、需要跨会话追踪或用户明确要求时，使用 `.scratch/<feature>/`，格式见 `docs/agents/issue-tracker.md`。
- handoff 按任务风险与执行需要补充步骤、检查清单和输出格式；这些要求只约束对应任务，不自动成为全仓默认规则。
- 跨实现、发布或数据操作的复杂任务，可在 handoff 中要求阶段 closure ledger，或使用 `Merged / Deployed / DataChanged / Verified / Next authorized` 五状态记录；仅列与任务相关的状态及其证据。
...
- 从实现转入发布、数据操作或新需求时，明确阶段变化、已有结果和下一步授权边界；授权已覆盖的工作继续执行，边界扩大时再取得授权。记录格式由对应 handoff 决定。
- context compaction 后，先恢复当前任务、已完成动作、验证证据和下一步授权，并核验下一步依赖的易变状态；不要求每次恢复都向用户输出固定 ledger。
- 新需求先明确范围与授权，不在验收尾部顺带实现；是否需要 ticket 按本文件“按任务补充记录与检查”处理。
- 请木下手工操作前 → 目的 / 影响 / 预期结果 / 回传物，四项前置说明。
- 进入部署、数据导入或批量重建等变更类阶段前 → 输出影响清单（涉及资源、账户级上限、数据域覆盖、回滚命令），逐项确认后执行。
