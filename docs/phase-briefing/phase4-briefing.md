...
不得在 Phase 4 对话中执行依赖升级、重新裁决已锁定架构、实现生产页面、接入真实投影或提前进入 Phase 5。
...
| Phase 4.3 | ✅ | 已完成获选原型视觉接口与参数的 canonical CSS 落回、五颗星球三槽身份闭环及双视口 UI QA；不代表生产 Home 已实现 |

## Canonical CSS 状态

- `src/styles/variables.css`：Phase 4.3 已建立 Space、Star Layer、Star Map、Planet Optical / Material / Overview / Focus、HAS、Leopard Cat 与 Cursor Meteor 的 canonical token 家族；runtime-owned 值不进入 CSS。
- `src/styles/components.css`：已建立 Home 深空舞台、Star Map、Planet、Focus、side index、HAS、豹猫星座、About Expanded 与 Cursor Meteor 的 selector / `data-*` 状态接口；未实现页面或 JavaScript 状态机。
- `src/styles/typography.css`：已补齐 `zh` / `zh-Hans` / `zh-CN` 一致性，以及 Planet label、Focus copy / action、side index 与 About 正文的排版接口。
- `src/styles/main.css`：已同步 v2.1 canonical 入口说明，导入顺序不变。
- PostCSS 语法解析、token 引用检查与 Astro build 已通过；详情见 `docs/design/qa/phase4-3-ui-qa.md`。
...
- About：例外；总览直接点击可轻推近后连续原地展开，自然滚动进入 About Focus 时也有明确 action；桌面豹猫使用两次独立点击，触控单次激活，进入同一展开态，不使用浏览器 `dblclick`。
...
- Phase 4.2 的五颗旧星球继续作为原型历史占位，不进入 selected slot。
- 五颗同系列星球已完成 Overview / Focus / Mobile 身份确认与标准化，manifest 位于 `docs/design/assets/planets/planet-asset-manifest.md`。
- 获选组件样式与适合 CSS 的校准参数已落回 canonical CSS；滚动、轨道、随机星场与豹猫物理继续保留为运行时契约。
- CJK、keyboard、touch、reduced-motion、性能、视觉一致性、1366×768 与 390×844 已进入 `docs/design/qa/phase4-3-ui-qa.md`。
- 生产 Home、真实 HAS 投影和正式资源加载策略仍属于 Phase 5，不得由本 brief 推断为已实现。
