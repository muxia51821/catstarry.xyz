...
## 设计基调 [定向回流中 | Phase 4.1 回归重锁]

> `DESIGN.md` v1.4（当前 11 节）仍是已有设计系统的事实来源；ADR-005、ADR-006 已锁定 Home / Feed 的架构边界。三画布、Klein Blue 与 CJK 原则可继续作为已确认设计原子；现在返回 Phase 4.1，重锁 Home 星图与 Feed 的视觉、交互和原型边界。

### 三画布系统

- **Home (Sci-Fi Editorial)**：冷调深黑画布 `#0A0A0C`，Klein Blue `#002FA7` 为唯一品牌色。Home 是 SSG 宇宙入口 → 接近星域 → 自由星图总览 → 页脚；About 在 Home 原地展开，不承担跨模块内容聚合，也不调用聚合 API。星图的具体视觉与交互接口在 Phase 4.1 回归重锁。
...
## 开发状态 [快照 | Phase 4.1 回归重锁中]

> 全局 Phase 3 已完成（2026-07-05）。Home / Feed 因 Phase 4.1 设计确认触发定向回流：定向 Phase 2、定向 Phase 3 均已完成并提交。当前返回 Phase 4.1 重锁；Phase 4.2 尚未启动。

- /blog：🟡 原型已上线，Phase 5 重做
- /：✅ 星图入口需求与 SSG 无聚合架构已锁定；🟡 Phase 4.1 重锁中；🔴 未开发
- /feed：✅ 公开足迹需求与分存事件架构已锁定；🟡 Phase 4.1 重锁中；🔴 未开发
