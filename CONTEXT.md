...
> `DESIGN.md` v2.0 是当前全站视觉与交互事实来源。Home Activity Signal 已完成定向 Phase 2/3 与返回 Phase 4.1 视觉重锁：ADR-007 锁定 Home 可消费无内容的三态静态投影，`DESIGN.md` 与 canonical CSS 已定义三态信号卫星的视觉和 token 接口。Phase 4.2 可用模拟状态恢复原型验证。
...
## 开发状态 [快照 | HAS 返回 Phase 4.1 已完成，Phase 4.2 可恢复]

> 全局 Phase 3、Home / Feed 定向 Phase 2 与定向 Phase 3 均已完成。Astro 7 定向依赖基线迁移已完成并经流程治理确认。Home Activity Signal 的定向 Phase 2/3 与返回 Phase 4.1 视觉重锁也已完成：PRD、HAS-01～HAS-03、ADR-007、`DESIGN.md` 与 canonical CSS 已对齐。当前唯一正确入口是 Phase 4.2 隔离原型与参数校准；4.2 只能使用模拟 activity states，不得接入真实投影。
